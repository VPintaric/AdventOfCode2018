...
constexpr int minAttack = 3;
constexpr int maxAttack = initialHealth;
...
struct Grid {
    ...
    Grid( const Grid &grid ) : width( grid.width ), height( grid.height ) {
...
EntityVector findEntities( const Grid &grid, int elfAttack, int goblinAttack, int &nElves, int &nGoblins ) {
    nElves = nGoblins = 0;
    ...
    for( int y = 0; y < grid.height; ++y ) {
        for( int x = 0; x < grid.width; ++x ) {
            if( data[ y ][ x ] == 'E' || data[ y ][ x ] == 'G' ) {
                ...
                if( data[ y ][ x ] == 'E' ) {
                    entity->attack = elfAttack;
                    entity->type = Entity::Type::Elf;
                    ++nElves;
                } else {
                    entity->attack = goblinAttack;
...
bool getBattleResult( const Grid &initialGrid, int elfAttack, int goblinAttack, int &finalScore ) {
    Grid grid( initialGrid );
    int nElves, nGoblins;
    auto entities = findEntities( grid, elfAttack, goblinAttack, nElves, nGoblins );
    int nElvesAtStart = nElves;
...
int main( void ) {
    ...
    while( lo < hi ) {
        int attack = (lo + hi) / 2;
        if( getBattleResult( grid, attack, minAttack, score )) {
            hi = attack;
        } else {
            lo = attack + 1;
        }
    ...
    getBattleResult( grid, lo, minAttack, score );
